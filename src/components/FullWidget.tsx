    ...
    <div className="flex flex-col h-full gap-1.5">
      ...
      <div className="flex items-center justify-around px-2 py-0.5 border-b border-pixel-border/30">
        <div className="text-center">
          <span className="text-[7px] text-pixel-xp font-pixel block">{totalCompleted}</span>
          <span className="text-[5px] text-pixel-dim font-pixel">COMPLETED</span>
        </div>
        <div className="text-center">
          <span className="text-[7px] text-pixel-hp font-pixel block">
            {doneToday}/{totalToday}
          </span>
          <span className="text-[5px] text-pixel-dim font-pixel">TODAY</span>
        </div>
        <div className="text-center">
          <span className="text-[7px] text-pixel-rare font-pixel block">
            {streak > 0 ? `🔥${streak}` : '—'}
          </span>
          <span className="text-[5px] text-pixel-dim font-pixel">STREAK</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-2">
        {activeTasks.length > 0 && (
          <div className="mb-1">
            <div className="text-[7px] text-pixel-accent font-pixel mb-0.5 tracking-wider">
              ── ACTIVE ──
            </div>
            <TaskList
              tasks={activeTasks}
        ...
        {completedTasks.length > 0 && (
          <div>
            <div className="text-[7px] text-pixel-hp font-pixel mb-0.5 tracking-wider">
              ── COMPLETED ──
            </div>
            <TaskList
            ...
            {completedTasks.length > 5 && (
              <p className="text-[6px] text-pixel-dim font-pixel text-center mt-0.5">
      ...
      <div className="absolute top-10 left-1/2 -translate-x-1/2 pointer-events-none z-50">
        {notifications.map(n => (
          <div
            key={n.id}
            className="animate-xp-float text-[10px] text-pixel-xp font-pixel whitespace-nowrap"
          >
            +{n.amount} EXP
          </div>
        ))}
